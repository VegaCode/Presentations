...
// The "normal" size of the presentation, aspect ratio will be preserved
	...
	function checkCapabilities() {
		...
		isMobileDevice = /(iphone|ipod|ipad|android)/gi.test( navigator.userAgent );// code from 2015
			...
			for( var i = 0, len = slides.length; i < len; i++ ) {
				...
				if( config.center || slide.classList.contains( 'center' ) ) {
					...
					if( slide.classList.contains( 'stack' ) ) {
						...
						// edited by Cesar Vega 9/16/2015
						// Desc: the top position never exceds below -46 px 
						slide.style.top =( Math.max( - ( getAbsoluteHeight( slide ) / 2 ) - (slidePadding, -slideHeight / 2) ) < - 41 ) ? -41 + 'px'  : Math.max( - ( getAbsoluteHeight( slide ) / 2 ) - (slidePadding, -slideHeight / 2) ) + 'px';
